...
- A learning experience in building, scaling, and testing a containerized API

# How to Use

## Populate your local MongoDB

- Use mongoimport to create tables
- Set URI in database/index.js to point to local mongo so you can test before containerization
- If a CSV contains errors that break mongoimport, use the included line-by-line loader code
- Use mongodump to create a dump directory

## Deploy containerized DB

- Create AWS instance and install docker, docker-compose
- Clone this repo
- SCP transfer the mongo dump directory into sdc directory
- Remove lines 4 - 11 from docker-compose.yml
- Run the docker-compose.yml with docker-compose up -d
- shell into docker container and mongorestore dump to populate the containerized DB
- Add authentication to your DB
- Add TCP port 27017 to allowed connections

## Deploy containerized server

- Create AWS instance, install docker, docker-compose
- Clone repo
- Remove lines 12-23 from docker-compose.yml
- Edit uri in database/index.js to point to your deployed DB and add your credentials
- Run the docker-compose.yml with docker-compose up -d
- Add HTTP port 80 to allowed connections
...
http {
    upstream backend {
        server 18.117.251.200:80
        server 18.216.33.123:80
        server 3.128.201.46:80
    }
    server {
        listen 80;
        location / {
...
# Trying NGINX Load Balancer

### 3 Instance NGINX 500/sec 60s

- 0 / 30000 15-242 ms avg 18

### 3 Instance NGINX 1000/sec 60s

- 17754 Network errors, 42245 success 23-3994 ms avg 277 Leveloff after 1 sec at 270 ms

- 17544 network errors, 42456 success 15-3551 ms avg 279 Leveloff after 1 sec at 280 ms

### 6 Instance NGINX 1000/sec 60s

- 19706 network errors, 40294 success
- 20051 network errors, 39949 success
