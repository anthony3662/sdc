...
app.get('/loadStyles', (req, res) => {

  var readPromise = readFile('./DATAIMPORT/styles.csv', 'utf8')
  .then(async function(raw) {
    var array = csv.toObjects(raw);
    console.log('read styles complete');
    for (var i = 0; i < array.length; i++) {
      if(i % 1000 === 0) {
        console.log('Entries saved:', i);
      }
      await style.save(array[i]);
    }
    console.log('write styles complete');
  });
});
